...
const ScrollResponsiveObject = ({ 
  ...
  if (shape === "box") {
    return (
      <Box ref={meshRef} args={[1, 1, 1]} position={[position[0], position[1], position[2]]} scale={scale}>
        <meshStandardMaterial color={color} transparent opacity={0.7} />
      </Box>
    );
  } else if (shape === "torus") {
    return (
      <Torus ref={meshRef} args={[1, 0.4, 16, 32]} position={[position[0], position[1], position[2]]} scale={scale}>
    ...
    // Default to sphere
    return (
      <Sphere ref={meshRef} args={[1, 16, 16]} position={[position[0], position[1], position[2]]} scale={scale}>
